...
def Face():
    ...
    image_paths = [os.path.join(path, f) for f in os.listdir(path) if f.endswith('.surprised')]
...
def Menu():
    ...
    btn = Button(window, text="Нахождение удивленных людей", padx=10, pady=7, command =Face, bg='#7CFFA8')  
    btn.pack(anchor="center", padx=50, pady=40)

    btn1 = Button(window, text="Выход", padx=10, pady=7, command =exit, bg='#7CFFA8')  
    btn1.pack(anchor="center", padx=50, pady=10)
